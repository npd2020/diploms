...
// ********************************************************************
//
/// \file G4SabatPrimaryGeneratorAction01.cpp
/// \brief Implementation of the G4SabatPrimaryGeneratorAction01 class

#include "G4SabatPrimaryGeneratorAction01.hh"
...
G4SabatPrimaryGeneratorAction01::G4SabatPrimaryGeneratorAction01()
    ...
    , fEnvelopeBox(0)
{
    ...
    listParticle.append(new G4ParticleGun(n_particle));
...
void G4SabatPrimaryGeneratorAction01::GeneratePrimaries(G4Event* anEvent)
{
    ...
    if (!fEnvelopeBox)
    {
        ...
        G4LogicalVolume* envLV = G4LogicalVolumeStore::GetInstance()->GetVolume("BottomExmpl");
        if (envLV)
    ...
    { //Если ссылка на fEnvelopeBox пустая, выдаем предупреждение
    ...
    listParticle.first()->SetParticlePosition(G4ThreeVector(x0, -y0, z0));
    // Генерируем первичное событие
    fParticleGun->GeneratePrimaryVertex(anEvent);
    //    listParticle.first()->GeneratePrimaryVertex(anEvent);
}
